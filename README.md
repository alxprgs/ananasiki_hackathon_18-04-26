...
Прошивка в [arduino/arduino.cpp](arduino/arduino.cpp) специально сделана как низкоуровневый слой без "умной" логики лабиринта.
...
- если Raspberry Pi перестал присылать команды дольше, чем `COMMAND_WATCHDOG_MS`, прошивка вызывает `stopAllMotion()`;
- это останавливает оба DC-мотора и шаговый двигатель;
...
- если передан `duration_ms`, остановка произойдёт автоматически на стороне Arduino;
...
- timed-команды учитываются по заданной длительности;
...
1. Открыть [arduino/arduino.cpp](arduino/arduino.cpp) и при необходимости поправить `Config`.
